...
public class FragmentationLogic {
	...
	public static ArrayList<HashMap<String, String>> calculate(int packLength, int frameInput, int header, String id) {
		...
		Integer frame = 0;
		Integer offset = 0;
		// paddingBytes prüft ob headerLength ein vielfaches von 4 ist.
		Integer paddingBytes = headerLength % 4;
		
		/**
		 * Falls headerLength kein vielfaches von 4 ist, wird der Header per padding aufgefüllt.
		 * Dies wird über eine switch-Anweisung über die headerLength ausgeführt. Danach enthält die Variable
		 * 'paddingBytes' die Anzahl der aufgefüllten Bytes und kann in der Visualisierung ausgegeben werden.
		 */
		if(paddingBytes != 0) {
			switch(paddingBytes) {
